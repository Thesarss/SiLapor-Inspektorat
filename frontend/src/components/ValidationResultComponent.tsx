...
export default function ValidationResultsComponent({
  ...
  const validateData = async () => {
    try {
      ...
      // Filter out completely empty rows before sending to backend
      const filteredRows = uploadData.preview.rows.filter((row: any) => {
      ...
      // Use the backend mapping service instead of manual mapping
      const response = await apiClient.post('/imports/validate-data-with-mapping', {
        data: filteredData,
